...
public class LoanStatementController {
    ...
    @FXML public void initialize() {
        ...
        paymentTable.setFixedCellSize(34);
        paymentTable.setMinHeight(84);
    ...
    private void loadPayments(Connection c, BigDecimal repayable) throws SQLException {
        ...
        paymentTable.setItems(rows); resizePaymentTable(); BigDecimal paid=rows.stream().filter(x->"PAID".equalsIgnoreCase(x.status)).map(x->BigDecimal.valueOf(x.amount)).reduce(BigDecimal.ZERO,BigDecimal::add); paidLabel.setText(money(paid)); repayableLabel.setText(money(repayable)); outstandingLabel.setText(money(repayable.subtract(paid).max(BigDecimal.ZERO)));
    }
    private void resizePaymentTable() { int rows = paymentTable.getItems() == null ? 0 : paymentTable.getItems().size(); paymentTable.setPrefHeight(Math.min(300, Math.max(84, 32 + rows * paymentTable.getFixedCellSize()))); }
    private void setStatus(Label label, String value) { label.setText(value); label.getStyleClass().removeIf(style -> style.startsWith("status-")); if (value != null && !"-".equals(value)) { label.getStyleClass().add("status-badge"); label.getStyleClass().add("status-" + value.toLowerCase().replace('_','-')); } }
